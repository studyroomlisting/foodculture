# reviews

**Description**: Customer reviews via the detail page

**TypeScript type**: `Review` (defined in `types/database.ts`)

**Key columns**: `id,restaurant_id,reviewer_name,rating,body`

**Used by**:
- See `FORM_TO_DATABASE_MAPPING.md` for all form writes to this table
- See `lib/queries.ts` for all read queries

**RLS**: See `DATABASE_SCHEMA.md` — section for this table
