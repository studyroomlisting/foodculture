...
export default function AdminDashboard({ initialTab = 'listings' }: { initialTab?: string }) {
  ...
  async function loadAll() {
    setLoading(true)
    const [{ data: l }, { data: u }, { data: r }, { data: e }, { data: a }] = await Promise.all([
    ...
    setListings(l ?? []); setUsers(u ?? []); setReviews(r ?? []); setEnquiries(e ?? [])
  ...
  const tabCounts: Record<Tab, number> = {
    ...
    users:       users.length,
    influencers: users.filter((u:any)=>u.role==='influencer').length,
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px' }}>
        ...
        {loading ? (
          ...
          <>
            ...
            {/* INFLUENCERS (FREELANCERS) */}
            {tab === 'influencers' && (
              <div>
                <div style={{fontSize:13,color:'#888',marginBottom:16}}>
                  Registered food creators — {users.filter((u:any)=>u.role==='influencer').length} total
                </div>
                {users.filter((u:any)=>u.role==='influencer').length===0
                  ? <div style={{textAlign:'center',padding:40,color:'#aaa',fontSize:14}}>No influencers registered yet.</div>
                  : users.filter((u:any)=>u.role==='influencer').map((u:any) => (
                    <div key={u.id} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 0',borderBottom:`1px solid ${C.border}`}}>
                      <div style={{width:36,height:36,borderRadius:'50%',background:'#F3EFFE',color:'#7F77DD',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:700,fontSize:14,flexShrink:0}}>
                        {(u.full_name||'I').charAt(0).toUpperCase()}
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:13,fontWeight:700}}>{u.full_name||'—'}</div>
                        <div style={{fontSize:11,color:'#888',marginTop:2}}>
                          {u.instagram_handle?`@${u.instagram_handle}`:u.username?`@${u.username}`:'No handle'} · Joined {new Date(u.created_at).toLocaleDateString('en-IN',{month:'short',year:'numeric'})}
                        </div>
                      </div>
                      <span style={{fontSize:11,padding:'2px 8px',borderRadius:8,background:u.suspended_at?'#fef2f2':'#EAF8EE',color:u.suspended_at?'#dc2626':C.green,fontWeight:600,flexShrink:0}}>
                        {u.suspended_at?'Suspended':'Active'}
                      </span>
                      <div style={{display:'flex',gap:6,flexShrink:0}}>
                        <Link href={`/admin/users/${u.id}`} style={{fontSize:11,background:'#F3EFFE',border:'1px solid #d4d0f5',borderRadius:6,padding:'4px 10px',color:'#7F77DD',fontWeight:600,textDecoration:'none'}}>Manage →</Link>
                        <button onClick={() => adminUserAction(u.id, u.suspended_at ? 'activate' : 'suspend')}
                          style={{fontSize:11,background:u.suspended_at?'#EAF8EE':'#FEF9EA',border:`1px solid ${u.suspended_at?'#b6e8c4':'#f5d5a0'}`,borderRadius:6,padding:'4px 10px',color:u.suspended_at?C.green:C.amber,cursor:'pointer',fontFamily:'inherit'}}>
                          {u.suspended_at ? 'Activate' : 'Suspend'}
                        </button>
        ...
        <div onClick={() => setConfirm(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 16, padding: 28, width: 360, maxWidth: '90vw' }}>
            <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 8 }}>
              {confirm.action === 'delete_review' ? 'Remove review?' : `${confirm.action.replace('_', ' ')} listing?`}
            </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={() => { if (confirm.action === 'delete_review') deleteReview(confirm.id); else if (confirm.action.startsWith('claim_')) updateClaimStatus(confirm.id, confirm.action.replace('claim_',''), confirm.id); else updateListingStatus(confirm.id, confirm.action, confirm.name) }}
                style={{ flex: 1, background: confirm.action === 'approved' ? C.green : C.red, color: '#fff', border: 'none', borderRadius: 10, padding: '10px 0', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
                Confirm
              </button>
            </div>
          </div>
